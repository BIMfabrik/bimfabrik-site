...
let visible=true,frame=0,previous=0;new IntersectionObserver(([entry])=>{visible=entry.isIntersecting;if(visible)requestFrame();},{threshold:.01}).observe(stage);function requestFrame(){if(!frame&&visible&&!document.hidden&&!reduced)frame=requestAnimationFrame(animate);}function animate(now){frame=0;const delta=Math.min((now-previous)/1000||0,.05);previous=now;if(!dragging)azimuth+=delta*.055;trains.forEach(train=>{const progress=(now*train.speed+train.phase)%1;train.electrons.forEach(electron=>{const travel=(progress-electron.userData.offset+1)%1;electron.position.copy(train.path.getPointAt(travel));electron.position.y+=.18;const size=1+Math.sin(now*.008+electron.userData.offset*28)*.17;electron.scale.setScalar(size);});train.lights.forEach(({light,index})=>light.position.copy(train.electrons[index].position));});render();requestFrame();}document.addEventListener("visibilitychange",()=>{if(!document.hidden){previous=performance.now();requestFrame();}});updateCamera();resize();requestFrame();
